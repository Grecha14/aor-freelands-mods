...
# The games hand-written speeds contradict weight: a glaive of eight kilos swings
# faster than a greatsword of five. One law instead, fitted to the games own ladder.
def speed(w):
    return 4.98/(5.22+M.kg(w,P))
...
def dps(cls,acls,slot="chest"):
    ...
    bleed=min(MAXST,speed(w)*DUR)*hit*cut*RATE if cut>0 else 0.0
    return direct+bleed
...
for c in cl:
    ...
    print("  %-17s %4.1f %5.2f %-6s"%(c,M.kg(w,P),speed(w),kind)+"".join(("%.1f"%v).rjust(8) for v in vals))
